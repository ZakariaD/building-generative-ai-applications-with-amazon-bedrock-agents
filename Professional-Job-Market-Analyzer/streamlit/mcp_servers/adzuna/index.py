...
def _get_adzuna_credentials():
    ...
    try:
        ...
        secret_name = os.getenv('ADZUNA_SECRET_NAME')
        
        if not secret_name:
            raise ValueError("ADZUNA_SECRET_NAME environment variable not set")
        
        response = secrets_client.get_secret_value(SecretId=secret_name)
        secret = json.loads(response['SecretString'])
        
        return secret['ADZUNA_APP_ID'], secret['ADZUNA_APP_KEY']
...
def get_adzuna_salary_chart(keywords: str, location: str = 'us', location0: str = None, location1: str = None, save_path: str = None) -> str:
    ...
    try:
        ...
        if not save_path:
            ...
            save_path = os.path.join('/app/charts', filename)
        ...
        logger.error(f"Error generating salary chart: {e}")
        return f"Error: {e}"
