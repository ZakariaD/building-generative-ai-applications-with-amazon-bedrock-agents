...
logger = logging.getLogger("Professional-Job-Market-Analyzer-V3")

# Get gateway information from environment variables
ADZUNA_GATEWAY_NAME = os.getenv('ADZUNA_GATEWAY_NAME')
USAJOBS_GATEWAY_NAME = os.getenv('USAJOBS_GATEWAY_NAME')
ADZUNA_GATEWAY_URL = os.getenv('ADZUNA_GATEWAY_URL')
USAJOBS_GATEWAY_URL = os.getenv('USAJOBS_GATEWAY_URL')

# Get MCP endpoints from environment or use defaults
ADZUNA_MCP_ENDPOINT = ADZUNA_GATEWAY_URL or os.getenv('ADZUNA_MCP_ENDPOINT')
USAJOBS_MCP_ENDPOINT = USAJOBS_GATEWAY_URL or os.getenv('USAJOBS_MCP_ENDPOINT')


# Log gateway information for debugging
print(f"ADZUNA_GATEWAY_NAME: {ADZUNA_GATEWAY_NAME}")
print(f"USAJOBS_GATEWAY_NAME: {USAJOBS_GATEWAY_NAME}")
print(f"ADZUNA_GATEWAY_URL: {ADZUNA_GATEWAY_URL}")
print(f"USAJOBS_GATEWAY_URL: {USAJOBS_GATEWAY_URL}")
...
# Connect to HTTP MCP servers via gateways using streamable HTTP transport
try:
    adzuna_client = MCPClient(lambda: create_streamable_http_transport(ADZUNA_MCP_ENDPOINT)) if ADZUNA_MCP_ENDPOINT else None
    
    usajobs_client = MCPClient(lambda: create_streamable_http_transport(USAJOBS_MCP_ENDPOINT)) if USAJOBS_MCP_ENDPOINT else None
